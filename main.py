...
gen_code = globals.generated_code  # This was missing from the aliases
...
def python_to_assembly(source_code, output_file, error_handler_func):
    ...
    try: # Gestione globale degli errori
        ...
        # --- Initialize the compiler's core components ---
        func_c = func_core.FuncCore()
        func_op = func_core.FuncCore()
        func_ex = func_core.FuncCore()
        func_str = func_core.FuncCore()
        func_d = func_core.FuncCore()
        func_c64_inst = func_core.FuncCore()
        func_b = func_core.FuncCore()
        c64_rout_lib = func_core.FuncCore()
        ast_proc = ast_processor.AstProcessor(func_ex, func_c, func_d, func_c64_inst, func_b) # pass the func_expressions instance
        func_struct = func_structures.FuncStructures(ast_proc, func_c, func_op, func_ex, func_str, func_d, func_c64_inst, func_b, c64_rout_lib, globals)  # Initialize with instances
        # Now, pass these instances around as needed. For example:
        # ast_proc = ast_processor.AstProcessor(func_ex, func_c, func_d, func_c64_inst, func_b) # pass the func_expressions instance
        # func_struct = func_structures.FuncStructures(ast_proc, func_c, func_op, func_ex, func_str, func_d, func_c64_inst, func_b, c64_rout_lib, globals)  # Initialize with instances

        # ast_processor = ast_processor.AstProcessor()
        ...
        for node in tree.body:
            ...
            try:
                if isinstance(node, ast.Assign):
                    ...
                    func_struct.process_if_node(node, error_handler_func)
                elif isinstance(node, ast.For):
                    func_struct.process_for_node(node, error_handler_func)
                elif isinstance(node, ast.While):
                    func_struct.process_while_node(node, error_handler_func)
