...
class Py2C64Compiler:
    ...
    def compile_code(self, python_code: str) -> str:
        ...
        try:
            ...
            # 2. Code Generation
            ast_tree.accept(self.code_generator)
            
            # 3. Add required routines from the code generator
    ...
    def _add_required_variables(self):
        ...
        # General variables
        self.output.add_lines(["RETURN_VALUE:   .word 0"])

        # Temporary variables from the code generator
        for i in range(self.code_generator.temp_var_counter):
            self.output.add_line(f"TEMP_{i}:        .word 0")
        
        # Global variables from the program's symbol table
